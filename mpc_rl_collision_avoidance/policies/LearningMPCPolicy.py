...
class LearningMPCPolicy(Policy):
    def __init__(self):
    ...
    def network_output_to_action(self, id, agents, network_output):
        ...
        if np.linalg.norm(agent.pos_global_frame - agent.goal_global_frame) < self.FORCES_N * self.dt * 2.0:
            ...
            return np.array([self.FORCES_x0[9*self.FORCES_TOTAL_V + self.FORCES_NU]-agent.pos_global_frame[0],
                             self.FORCES_x0[9 * self.FORCES_TOTAL_V + self.FORCES_NU+1]-agent.pos_global_frame[1]])
        else:
