...
def gaussian_likelihood_clipped(input_, mu_, std):
    """
    ...
    :return: (tf.Tensor)
    """
    pre_sum = 0.5 * (((input_ - mu_) / (tf.maximum(std,0.1) + EPS)) ** 2 + 2 *tf.log(tf.maximum(std,0.1)) + np.log(2 * np.pi))
...
class FeedForwardPolicy(SACPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False):
        ...
        #return self.sess.run(self.act_mu, {self.obs_ph: obs})
