...
def dynamic_lstm(ego_agent_features,other_agents_features, mask_tensor, cell_state_hidden, scope, n_hidden, seq_length,init_scale=1.0, layer_norm=False):
    """
    Creates an Long Short Term Memory (LSTM) cell for TensorFlow

    :param ego_agent_features: (TensorFlow Tensor) The input tensor with ego-agent features
    :param other_agents_features: (TensorFlow Tensor) The input tensor with other-agents features
    ...
    cell_state, hidden_state = tf.split(axis=1, num_or_size_splits=2, value=cell_state_hidden)
    """Reset State if batch is in the sequence dimension
        for idx, (_input, mask) in enumerate(zip(input_tensor, mask_tensor)):
            cell_state = cell_state * (1 - mask)
            hidden = hidden * (1 - mask)
    """
    """TODO: Reset State """
    cell_state = tf.unstack(cell_state,axis=0)
    hidden_state = tf.unstack(hidden_state,axis=0)
    mask_tensor = tf.unstack(mask_tensor,axis=0)
    for idx, (state,hidden, mask) in enumerate(zip(cell_state,hidden_state, mask_tensor)):
        cell_state[idx] = state * (1 - mask)
        hidden_state[idx] = hidden * (1 - mask)
    cell_state = tf.stack(cell_state,axis=0)
    hidden_state = tf.stack(hidden_state,axis=0)
    mask_tensor = tf.stack(mask_tensor,axis=0)
    with tf.variable_scope(scope):
        init_state_tuple_lstm = tf.contrib.rnn.LSTMStateTuple(cell_state, hidden_state)
        cell_ped = tf.nn.rnn_cell.LSTMCell(n_hidden, name='basic_lstm_cell', trainable=True)
        output, cell_state_hidden = tf.nn.dynamic_rnn(cell_ped,
                                                      other_agents_features,
                                                      dtype=tf.float32,
                                                      sequence_length=seq_length,
                                                      initial_state=init_state_tuple_lstm)
    cell_state, hidden = cell_state_hidden
    cell_state_hidden = tf.concat(axis=1, values=[cell_state, hidden])
    #output = tf.concat(axis=1, values=[ego_agent_features, hidden])
    regularizer = tf.contrib.layers.l2_regularizer(scale=0.0)
    output = tf.concat(axis=1, values=[ego_agent_features, hidden])
    layer1 = tf.layers.dense(inputs=output, units=256, activation=tf.nn.relu,
                                  kernel_regularizer=regularizer, name='layer1')

    layer2 = tf.layers.dense(inputs=layer1, units=256, activation=tf.nn.relu, name='layer2')
    final_flat = tf.contrib.layers.flatten(layer2)

    return final_flat, cell_state_hidden
