...
import sys
sys.path.append('/home/amr/code/rl-project/gym-collision-avoidance')
import sys
import argparse
...
def main():
    ...
    plot_save_dir = log_path + '/figs/only_mpc/'
    ...
    for ep_id in tqdm(range(args.n_episodes)):
        ...
        agents[0].policy.only_mpc = False
        ...
        while not done:
            ...
            action, exit_flag = agents[0].policy.mpc_output(0,agents)
            ...
            agents[0].policy.network_output_to_action(0, agents, action[0])
    ...
    results_file = stats_path + '_MPC_model_'+str(args.exp_id)+'_'+str(args.n_agents)+'_agents_perf_results.mat'
    ...
    with open(os.path.join(stats_path, 'model_'+str(args.exp_id)+'_'+str(args.n_agents)+'_mpc_agents_perf_results.yml'), 'w') as f:
