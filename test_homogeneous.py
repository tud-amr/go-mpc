...
import sys
sys.path.append('/home/bdebrito/code/rl-project/gym-collision-avoidance')
import sys
import argparse
...
def main():
    ...
    parser.add_argument('--n-envs', help='number of environments', default=1,
                        type=int)
    parser.add_argument('--n-agents', help='number of agents', default=3,
                        type=int)
    parser.add_argument('--exp-id', help='Experiment ID (default: -1, no exp folder, 0: latest)', default=26,
                        type=int)
    ...
    Config.HOMOGENEOUS_TESTING = True
    env, one_env = create_env()

    #agents = tc.go_to_goal(0)
    one_env.scenario = [args.scenario]
    env.unwrapped.envs[0].env.number_of_agents = args.n_agents
    env.unwrapped.envs[0].env.ego_policy = args.policy

    obs = env.reset()
    policies = []
    if 'MPCRLPolicy' in str(args.policy):
        for j in range(args.n_agents*2):
            policies.append(PPO2MPC.load(model_path, env=env))
        for i,agent in enumerate(env.unwrapped.envs[0].env.agents):
            agent.policy.policy_network = policies[i]
            agent.policy.is_still_learning = False
            agent.policy.enable_collision_avoidance = args.coll_avd
    ...
    plot_save_dir = log_path + '/figs/homogeneous/'+str(args.policy)+"/"
    ...
    for ep_id in tqdm(range(args.n_episodes)):
        ...
        episode_step = 0
        for id in range(args.n_agents * 2):
            states.append(None)
        n_infeasible= 0
        while not done:
            ...
            if 'MPC' in str(args.policy):
                for i,agent in enumerate(env.unwrapped.envs[0].env.agents):
                    obs = env.unwrapped.envs[0].env.observation[i]
                    agent.policy.find_next_action(obs, agents, i)
    ...
    results_file = stats_path + str(args.policy)+'_HOMOGENEOUS_model_'+str(args.exp_id)+'_'+str(args.n_agents)+'_agents_perf_results.mat'
    ...
    with open(os.path.join(stats_path, 'model_'+str(args.exp_id)+'_'+str(args.n_agents)+'_homogeneous_agents_perf_results_'+str(args.policy)+'.yml'), 'w') as f:
